...
"""
    ...
    invx native OS utility call support.
...
import fnmatch
import subprocess
from invx.exceptions     import SubprocessError
from invx.utils.log      import logger
from invx.utils.compat   import console_to_str
...
# ignore a few problematic directories that trick the search for invx resources
...
def get_prog():
    try:
        if os.path.basename(sys.argv[0]) in ('__main__.py', '-c'):
            return "%s -m invx" % sys.executable
